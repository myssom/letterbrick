...
if st.button("🤖 AI 피드백 받기 (형태변형)", key="form1_btn"):
    with st.spinner("AI가 평가 중입니다..."):
        ...
        문장: "{form1}"
        """
        r1 = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": p1}])
        ...
        문장: "{form1}"
        """
        rp1 = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": prof1}])
...
if st.button("🤖 AI 피드백 받기 (창의적)", key="form2_btn"):
    with st.spinner("AI가 평가 중입니다..."):
        ...
        문장: "{form1}"
        """
        f1_eval = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": p1}]).choices[0].message.content

        # 🎓 형태변형 한 줄 평
        prof1 = f"문장: {form1}"
        pf1_line = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": prof1}]).choices[0].message.content
        ...
        문장: "{form2}"
        """
        r2 = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": p2}])
        ...
        # 🎓 창의적 한 줄 평
        prof2 = f"문장: {form2}"
        pf2_line = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": prof2}]).choices[0].message.content
